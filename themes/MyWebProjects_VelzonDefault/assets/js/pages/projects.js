...
// 		$( '#project_branch' ).val( predefinedProjects[type].branch );
	...
	$( '#sectionProjects' ).on( 'click', '.btnInstall', function( e )
	{
        ...
        const myModal = new bootstrap.Modal( '#project-install-modal', {
            keyboard: false
        });
        myModal.show( $( '#project-install-modal' ).get( 0 ) );
					...
					$.ajax({
						...
						success: function( response )
						{
							...
							if ( response.status == 'error' ) {
								...
								/** Bootstrap 5 Modal Toggle */
                                const myModal = new bootstrap.Modal( '#delete-project-modal', {
                                    keyboard: false
                                });
                                myModal.show( $( '#delete-project-modal' ).get( 0 ) );
		...
		$.ajax({
			...
			success: function( response )
			{
				...
				if ( response.status == 'error' ) {
					...
					/** Bootstrap 5 Modal Toggle */
                    const myModal = new bootstrap.Modal( '#create-project-modal', {
                        keyboard: false
                    });
                    myModal.show( $( '#create-project-modal' ).get( 0 ) );
